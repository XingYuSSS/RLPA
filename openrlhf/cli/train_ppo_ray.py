...
from openrlhf.datasets import MultiRoundDataset
...
def train(args):
    ...
    if args.use_multiturn:
        print('use_multiturn')
        multiround_dataset = MultiRoundDataset.options(name="multiround_dataset").remote(args.actor_num_nodes * args.actor_num_gpus_per_node, args.n_samples_per_prompt, args.max_rounds, args.remote_rm_url, args.system_file, args.round_batch_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_multiturn", type=bool, default=False, help="use multiturn"
    )
    parser.add_argument(
        "--max_rounds", type=int, default=1, help="number of rounds"
    )
    parser.add_argument(
        "--round_batch_size", type=int, default=1, help="number of rounds"
    )
    parser.add_argument(
        "--system_file", type=str, default=None, help="path of system prompt"
    )
    parser.add_argument(
        "--shuffle", type=bool, default=False, help="shuffle dataset"
    )
